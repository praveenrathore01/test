...
async function create(req, res, userParam) {
	...
	if (errors) {
		...
		if (await User.findOne({ email: userParam.email })) {
			res
				.status(200)
				.json({ status: false, error: 'Email ' + userParam.email + ' is already taken' });
		} else {
			...
			// save user
			await user.save().then(() => res.json({ status: true })).catch((err) => res.json(err));
		}
	}
}
